...
class QQApiHelper implements PlatformApi {
                ...
                httpApi.getUserInfo(appId, result.getToken(), result.getOpenId(),
                        new RspHandler() {
                            ...
                            public void onSuccess(String s) {
                                final UserInfo userInfo = UserInfo.fromJson(s, new QQUserInfoParser());

                                //获取QQ_UnionId
                                httpApi.getQQUnionId(result.getToken(),new RspHandler(){
                                    @Override
                                    public void onSuccess(String s) {
                                        QQUnionId unionIdObj = QQUnionId.parser(s);
                                        if (unionIdObj != null
                                                && !TextUtils.isEmpty(unionIdObj.getUnionid())) {
                                            userInfo.setUnionid(unionIdObj.getUnionid());
    ...
    private IUiListener dispatchShareListener(final int type, @Nullable final OnShareListener shareListener){
        return new IUiListener() {
            ...
            public void onCancel() {
                if (shareListener != null) {
                    shareListener.onShareError(Platform.QQ);
                }
            }
        };
    }
